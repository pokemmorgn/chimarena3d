class CardModal {
  constructor(cardsTab) {
    this.cardsTab = cardsTab; // pour accéder à collection, allCards, authenticatedFetch, etc.
    this.modal = null;
  }
  ...
  open(cardOwned) {
    ...
    const info = cardOwned.cardInfo || this.cardsTab.allCards.find(c => c.id === cardOwned.cardId) || {};
    const lvl = cardOwned.level || 1;
    const max = cardOwned.maxLevel || info.maxLevel || 13;

    this.modal.querySelector(".cm-name").textContent = info.nameKey || cardOwned.cardId;
    ...
    // TODO : remplir stats, requirements upgrade, etc.
